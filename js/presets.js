...
function selectPreset(id) {
  ...
  // Build antibiotic panel
  state.antibiotics = panel.antibiotics.map(code => {
    const ab = ANTIBIOTICS_DB[code];
    return { id: code, name: ab.name, class: ab.class, brand: ab.brand, mic: '', sir: '', notes: '' };
  });
  ...
  orgSel.innerHTML = '<option value="">— Seleziona microrganismo —</option>' +
    ...
    '<option value="__custom__">✏️ Altro (inserisci manualmente)</option>';
